...
pub fn parse_config(base_dir: &str) -> Result<XeonitteConfig> {
    debug!("Parsing config {}/xeonitte/config.yml", base_dir);
    let f = fs::read_to_string(format!("{}/xeonitte/config.yml", base_dir))?;
    let mut config: XeonitteConfig = serde_yaml::from_str(&f)?;
    for choice in &mut config.choices {
        match choice {
            ChoiceEnum::Configuration { file, config } => {
                let f = fs::read_to_string(format!("{}/xeonitte/{}", base_dir, file))?;
...
pub fn parse_branding(base_dir: &str, brand: &str) -> Result<BrandingConfig> {
    let f = fs::read_to_string(format!(
        r#"{}/xeonitte/branding/{}/slides.yml"#,
        base_dir, brand
    ))?;
